...
export const compressChannel = async (channelId: number): Promise<boolean> => {
  const channel = await getChannel(channelId);
  if (!channel) return false;

  const rows = (await listMessageRows(channelId)).filter(
  ...
  const names = new Map((await listAgents()).map((a) => [a.id, a.name]));
  const transcript = toCompress
    .map((m) => `${m.author_type === "human" ? "Human" : (names.get(m.author_id ?? -1) ?? "Agent")}: ${m.content}`)
    .join("\n");

  const headKind = channel.head_agent_id ? (await getAgentById(channel.head_agent_id))?.providerKind : undefined;
  ...
  const pinned = (await setPinned(mem.id, true)) ?? mem;
  broadcast("memory:added", toMemory(pinned));
  await db.execute(from(channels).where((q) => q("id").equals(channelId)).update({ compressed_through: lastId }));
  return true;
};
