...
export const authRoutes = [
  post(
    ...
    pipe(async (c: Conn) => {
      ...
      const jwt = await token.sign({ sub: user.id }, config.authSecret, { expiresIn: MAX_AGE });
      return json(putHeader(c, "set-cookie", cookieHeader(jwt)), 200, { id: user.id, name: user.name, email: user.email });
    }),
  ),
  post("/api/logout", pipe((c: Conn) => json(putHeader(c, "set-cookie", `${COOKIE}=; HttpOnly; Path=/; Max-Age=0`), 200, { ok: true }))),
  get(
    "/api/me",
    pipeline(requireAuth)(async (c: Conn) => {
      const user = await db.one<{ id: number; name: string; email: string }>(
        from(users).where((q) => q("id").equals(c.assigns.userId as number)).select("id", "name", "email"),
      );
