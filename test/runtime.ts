...
import { channels, db, from, schedules } from "../src/db/index.ts";
...
const dm = await openDmRow(llama.id);
await addMemberRow(dm.id, llama.id);

let mark = events.length;
const hi = await insertMessage({ channelId: dm.id, authorType: "human", content: "[reply-plain] hello there" });
...
await drain();
{
  ...
  const rows = await listMessageRows(dm.id);
...
mark = events.length;
const dmsg = await insertMessage({ channelId: dm.id, authorType: "human", content: "[reply-directives] update yourself" });
...
const proj = await createProjectRow("Research Bay", "deep questions");
...
await addMemberRow(proj.id, maestro.id);
await setHeadAgent(proj.id, maestro.id);

mark = events.length;
const agentsBefore = (await listAgents()).length;
const ask = await insertMessage({ channelId: proj.id, authorType: "human", content: "What is the meaning of it all?" });
...
await drain();
{
  ...
  const rows = await listMessageRows(proj.id);
...
const archive = await createProjectRow("Archive", "long history");
await addMemberRow(archive.id, llama.id);
await setHeadAgent(archive.id, llama.id); // head=ollama so compress uses the fake provider
...
await createSchedule({ channelId: dm.id, agentId: llama.id, prompt: "give me a status", intervalMinutes: 1 }, realNow - 5 * 60_000);
const dmRowsBefore = (await listMessageRows(dm.id)).length;
mark = events.length;
await tick();
await drain();
{
  ...
  const dmRowsAfter = await listMessageRows(dm.id);
...
{
  const hits = await search("Tandem");
